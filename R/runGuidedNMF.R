...
runGuidedNMF <- function(x, k, label_col, mode = c("enrich", "remove"),
    ...
    if (identical(mode, "remove")) {
        stop("mode = \"remove\" is disabled: it produces a degenerate ",
             "factorization. RcppML's adversarial target removal collapses ",
             "the embedding onto a single program at every strength tested ",
             "(0.5 to 10), leaving most programs identically zero. For batch ",
             "correction use runConditionedNMF(), or refineNMF() with ",
             "batch_col, both of which preserve cell type structure.")
    }
